...
I think David Tolnay's blog post on [soundness bugs] draws a very important distinction between "unsafe" as it is used in Rust, and "unsound". Essentially, "unsound" means that all possible uses of a library avoid undefined behavior. This is quite a high standard, and in many cases goes beyond what's necessary to deliver working code. Even so, it is the standard that the Rust language (including standard library) and many libraries aspire to.
